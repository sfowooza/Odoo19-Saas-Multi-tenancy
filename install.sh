...
USE_INTERACTIVE=${USE_INTERACTIVE:-true}
...
configure_ports() {
    if [ "$USE_INTERACTIVE" = "false" ] || [ -t 0 ]; then
        # Check if user wants to configure ports
        print_status "Port Configuration"
        echo ""
        echo "Would you like to configure custom ports? (y/N)"
        read -r configure_ports_input
        if [[ $configure_ports_input =~ ^[Yy]$ ]]; then
            # Configure Odoo port
            echo ""
            echo "Enter main Odoo port (default: 7001):"
            read -r input_odoo_port
            if [ -n "$input_odoo_port" ]; then
                ODOO_PORT=$input_odoo_port
            fi

            # Configure pgAdmin port
            echo ""
            echo "Enter pgAdmin port (default: 5050):"
            read -r input_pgadmin_port
            if [ -n "$input_pgadmin_port" ]; then
                PGADMIN_PORT=$input_pgadmin_port
            fi

            # Configure chat port (optional)
            echo ""
            echo "Enter Odoo chat port (default: 7002, press Enter to skip):"
            read -r input_chat_port
            if [ -n "$input_chat_port" ]; then
                ODOO_CHAT_PORT=$input_chat_port
            fi
...
setup_configuration() {
    ...
    if [ ! -f ".env" ]; then
        cat > .env << EOF
# 🔧 Port Configuration for Odoo 19 SaaS Platform
# Auto-generated by installer

# Main Odoo Web Interface
ODOO_PORT=${ODOO_PORT:-7001}

# Odoo Chat/Longpolling Port (optional)
ODOO_CHAT_PORT=${ODOO_CHAT_PORT:-7002}
...
EOF
        ...
        # Update existing .env with any configured ports
        if [ -n "$ODOO_PORT" ]; then
            sed -i "s/^ODOO_PORT=.*/ODOO_PORT=$ODOO_PORT/" .env
        fi
        if [ -n "$ODOO_CHAT_PORT" ]; then
            sed -i "s/^ODOO_CHAT_PORT=.*/ODOO_CHAT_PORT=$ODOO_CHAT_PORT/" .env
        fi
        if [ -n "$PGADMIN_PORT" ]; then
            sed -i "s/^PGADMIN_PORT=.*/PGADMIN_PORT=$PGADMIN_PORT/" .env
        fi
    fi
...
get_odoo_port() {
    if [ -f ".env" ]; then
        local port=$(grep "^ODOO_PORT=" .env | cut -d'"' -f2)
...
get_pgadmin_port() {
    if [ -f ".env" ]; then
        local port=$(grep "^PGADMIN_PORT=" .env | cut -d'"' -f2)
